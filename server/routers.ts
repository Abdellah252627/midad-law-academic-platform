...
export const appRouter = router({
  ...
  forum: router({
    ...
    moderationCheck: protectedProcedure.input(z.object({ text: z.string().trim().min(1).max(10000) })).query(async ({ input }) => ({
      allowed: !(await findActiveForumBlockedTerm(input.text)),
