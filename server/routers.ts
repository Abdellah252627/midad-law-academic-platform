...
export const appRouter = router({
  ...
  forum: router({
    ...
    createTopic: protectedProcedure.input(z.object({ categoryId: z.number().int().positive(), title: z.string().trim().min(8, "العنوان قصير جداً").max(220), body: z.string().trim().min(20, "المحتوى قصير جداً").max(10000) })).mutation(async ({ input, ctx }) => ({ id: await createForumTopic({ ...input, authorUserId: ctx.user.id }), status: "pending" as const })),
    createReply: protectedProcedure.input(z.object({ topicId: z.number().int().positive(), body: z.string().trim().min(5, "الرد قصير جداً").max(5000) })).mutation(async ({ input, ctx }) => ({ id: await createForumReply({ ...input, authorUserId: ctx.user.id }), status: "pending" as const })),
  ...
  admin: router({
    ...
    moderateTopic: adminProcedure.input(z.object({ id: z.number().int().positive(), status: z.enum(["pending", "published", "hidden", "closed"]) })).mutation(async ({ input }) => { await updateForumTopicStatus(input.id, input.status); return { success: true as const }; }),
    moderateReply: adminProcedure.input(z.object({ id: z.number().int().positive(), status: z.enum(["pending", "published", "hidden"]) })).mutation(async ({ input }) => { await updateForumReplyStatus(input.id, input.status); return { success: true as const }; }),
