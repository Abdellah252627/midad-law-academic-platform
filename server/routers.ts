...
export const appRouter = router({
  ...
  admin: router({
    ...
    forumQueue: adminProcedure.input(z.object({ status: z.enum(["pending", "published", "hidden", "closed"]).optional(), search: z.string().trim().max(160).optional(), itemType: z.enum(["topic", "reply"]).optional() }).optional()).query(async ({ input }) => getForumModerationQueue(input ?? {})),
