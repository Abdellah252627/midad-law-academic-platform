...
export const appRouter = router({
  ...
  admin: router({
    ...
    forumQueue: adminProcedure.input(z.object({ status: z.enum(["pending", "published", "hidden", "closed"]).optional(), search: z.string().trim().max(160).optional(), itemType: z.enum(["topic", "reply"]).optional(), level: z.array(FORUM_LEVEL_SCHEMA).max(FORUM_LEVELS.length).optional() }).optional()).query(async ({ input }) => getForumModerationQueue(input ?? {})),
    forumReports: adminProcedure.query(async () => getOpenForumReports()),
    moderateTopic: adminProcedure.input(z.object({ id: z.number().int().positive(), status: z.enum(["pending", "published", "hidden", "closed"]), reason: z.string().trim().max(500).optional() })).mutation(async ({ input, ctx }) => { await updateForumTopicStatus(input.id, input.status); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.topic.moderate", entityType: "forum_topic", entityId: String(input.id), productCode: DEFAULT_PRODUCT_CODE, metadataJson: JSON.stringify({ status: input.status, reason: input.reason ?? null }) }); return { success: true as const }; }),
    moderateReply: adminProcedure.input(z.object({ id: z.number().int().positive(), status: z.enum(["pending", "published", "hidden"]), reason: z.string().trim().max(500).optional() })).mutation(async ({ input, ctx }) => { await updateForumReplyStatus(input.id, input.status); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.reply.moderate", entityType: "forum_reply", entityId: String(input.id), productCode: DEFAULT_PRODUCT_CODE, metadataJson: JSON.stringify({ status: input.status, reason: input.reason ?? null }) }); return { success: true as const }; }),
    reviewForumReport: adminProcedure.input(z.object({ id: z.number().int().positive(), status: z.enum(["reviewed", "dismissed"]) })).mutation(async ({ input, ctx }) => { await updateForumReportStatus(input.id, input.status, ctx.user.id); return { success: true as const }; }),
    blockedWords: adminProcedure.query(async () => getForumBlockedWords(true)),
    addBlockedWord: adminProcedure.input(z.object({ term: z.string().trim().min(2).max(120) })).mutation(async ({ input, ctx }) => { const id = await createForumBlockedWord({ term: input.term, createdByUserId: ctx.user.id }); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.blocked_word.create", entityType: "forum_blocked_word", entityId: String(id), productCode: DEFAULT_PRODUCT_CODE, metadataJson: JSON.stringify({ term: input.term }) }); return { id }; }),
    setBlockedWordActive: adminProcedure.input(z.object({ id: z.number().int().positive(), isActive: z.boolean() })).mutation(async ({ input, ctx }) => { await updateForumBlockedWord(input.id, { isActive: input.isActive }); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.blocked_word.toggle", entityType: "forum_blocked_word", entityId: String(input.id), productCode: DEFAULT_PRODUCT_CODE, metadataJson: JSON.stringify({ isActive: input.isActive }) }); return { success: true as const }; }),
    deleteBlockedWord: adminProcedure.input(z.object({ id: z.number().int().positive() })).mutation(async ({ input, ctx }) => { await deleteForumBlockedWord(input.id); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.blocked_word.delete", entityType: "forum_blocked_word", entityId: String(input.id), productCode: DEFAULT_PRODUCT_CODE }); return { success: true as const }; }),
    forumModerationBlocks: adminProcedure.query(async () => getActiveForumModerations()),
    forumViolationMonitoring: adminProcedure.input(z.object({ search: z.string().trim().max(160).optional(), includeResolved: z.boolean().default(false) }).optional()).query(async ({ input }) => getForumViolationMonitoring(input ?? {})),
    forumParticipantClassification: adminProcedure.input(z.object({ search: z.string().trim().max(160).optional() }).optional()).query(async ({ input }) => getForumParticipantClassification(input ?? {})),
    forumWeeklyViolationReport: adminProcedure.query(async () => getForumWeeklyViolationReport()),
    clearForumModerationBlock: adminProcedure.input(z.object({ userId: z.number().int().positive() })).mutation(async ({ input, ctx }) => { await clearForumModerationBlock(input.userId); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.user_moderation.clear_block", entityType: "forum_user_moderation", entityId: String(input.userId), productCode: DEFAULT_PRODUCT_CODE }); return { success: true as const }; }),
    resetForumViolationCounter: adminProcedure.input(z.object({ userId: z.number().int().positive() })).mutation(async ({ input, ctx }) => { await resetForumViolationCounter(input.userId); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.user_moderation.reset_counter", entityType: "forum_user_moderation", entityId: String(input.userId), productCode: DEFAULT_PRODUCT_CODE }); return { success: true as const }; }),
