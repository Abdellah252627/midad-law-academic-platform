...
export const appRouter = router({
  ...
  forum: router({
    ...
    moderationCheck: protectedProcedure.input(z.object({ text: z.string().trim().min(1).max(10000) })).query(async ({ input, ctx }) => {
      ...
      return {
        allowed: isForumOpenAt() && !status.isBlocked && !(await findActiveForumBlockedTerm(input.text)),
        warning: isForumOpenAt() ? FORUM_BLOCKED_WORD_WARNING : FORUM_CLOSED_MESSAGE,
