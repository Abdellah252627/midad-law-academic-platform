...
import { FORUM_BLOCKED_WORD_WARNING, FORUM_SENSITIVE_DATA_WARNING, findSensitiveForumData, redactSensitiveForumData } from "../shared/forumModeration";
import { FORUM_CLOSED_MESSAGE, FORUM_MODERATION_THRESHOLD, getForumModerationWarning, isForumOpenAt } from "../shared/forumModerationPolicy";
import { buildPurchaseRequestsXlsx, type PurchaseExportRow } from "./xlsxExport";
import { systemRouter } from "./_core/systemRouter";
import { adminProcedure, isAuthorizedAdmin, isPlatformOwner, ownerProcedure, protectedProcedure, publicProcedure, router } from "./_core/trpc";
import { TRPCError } from "@trpc/server";

const forumModeratorProcedure = protectedProcedure.use(async ({ ctx, next }) => {
  const isOwner = isAuthorizedAdmin(ctx.user);
  const isModerator = !isOwner && await isActiveForumModerator(ctx.user.id);
  if (!isOwner && !isModerator) {
    throw new TRPCError({ code: "FORBIDDEN", message: "لا تملك صلاحية الإشراف على المنتدى" });
  }
...
export const appRouter = router({
  ...
  auth: router({
    ...
    access: publicProcedure.query(async opts => {
      const user = opts.ctx.user;
      if (!user) return { isAuthenticated: false, isAdmin: false, isOwner: false, isForumModerator: false } as const;
      const isAdmin = isAuthorizedAdmin(user);
      const isOwner = isPlatformOwner(user);
      const isForumModerator = isAdmin || await isActiveForumModerator(user.id);
      return { isAuthenticated: true, isAdmin, isOwner, isForumModerator } as const;
    }),
  ...
  forum: router({
    rules: publicProcedure.query(() => ({ version: FORUM_RULES_VERSION, title: "قواعد منتدى Midad Law", items: [
      "النقاش تعليمي ومحترم، ويمنع السب والتشهير والتحريض والتنمر.",
      "يمنع نشر أرقام الهواتف والبريد والعناوين أو أي بيانات شخصية للغير.",
      "لا تُقدَّم المنشورات على أنها استشارة قانونية أو حكم نهائي في قضية شخصية.",
      "تخضع الموضوعات والردود للمراجعة، وقد تُخفى المشاركات المخالفة أو المضللة.",
      "باستخدام المنتدى، يوافق العضو على سياسة الخصوصية وقواعد الإشراف المعمول بها.",
    ] as const })),
    ...
    moderationCheck: protectedProcedure.input(z.object({ text: z.string().trim().min(1).max(10000) })).query(async ({ input, ctx }) => {
      ...
      const sensitive = findSensitiveForumData(input.text);
      const blockedTerm = await findActiveForumBlockedTerm(input.text);
      return {
        allowed: isForumOpenAt() && !status.isBlocked && !blockedTerm && !sensitive,
        warning: !isForumOpenAt() ? FORUM_CLOSED_MESSAGE : sensitive ? FORUM_SENSITIVE_DATA_WARNING : FORUM_BLOCKED_WORD_WARNING,
        forumOpen: isForumOpenAt(),
        forumHoursMessage: FORUM_CLOSED_MESSAGE,
        isBlocked: status.isBlocked,
        remainingMinutes: Math.ceil(status.remainingMs / 60000),
        violationCount: status.violationCount,
        remainingAttempts: warningState.remainingAttempts,
        nearLimitWarning: warningState.message,
        sensitiveDataCategory: sensitive?.category ?? null,
      };
    }),
    ...
    createTopic: protectedProcedure.input(z.object({ categoryId: z.number().int().positive(), subject: FORUM_SUBJECT_SCHEMA, level: FORUM_LEVEL_SCHEMA, title: z.string().trim().min(8, "العنوان قصير جداً").max(220), body: z.string().trim().min(20, "المحتوى قصير جداً").max(10000) })).mutation(async ({ input, ctx }) => { if (!isForumOpenAt()) throw new Error(FORUM_CLOSED_MESSAGE); if (!(await hasAcceptedForumRules(ctx.user.id))) throw new Error("يجب الموافقة على قواعد المنتدى قبل المشاركة"); const status = await getForumModerationStatus(ctx.user.id); if (status.isBlocked) throw new Error(`تم إيقاف المشاركة مؤقتاً. يمكنك المحاولة بعد نحو ${Math.max(1, Math.ceil(status.remainingMs / 60000))} دقيقة.`); const fullText = `${input.title}\n${input.body}`; const sensitive = findSensitiveForumData(fullText); if (sensitive) { await recordForumModerationViolation(ctx.user.id, new Date(), { sourceType: "topic", category: sensitive.category, text: fullText, redactedExcerpt: redactSensitiveForumData(fullText) }); throw new Error(FORUM_SENSITIVE_DATA_WARNING); } const blockedTerm = await findActiveForumBlockedTerm(fullText); if (blockedTerm) { const violation = await recordForumModerationViolation(ctx.user.id, new Date(), { sourceType: "topic", text: fullText, matchedTerm: blockedTerm }); if (violation.isBlocked) throw new Error(`تكرر نشر محتوى مخالف؛ تم إيقاف المشاركة مؤقتاً لمدة ${Math.ceil(violation.remainingMs / 60000)} دقيقة.`); throw new Error(`${FORUM_BLOCKED_WORD_WARNING} المحاولة ${violation.violationCount} من ${FORUM_MODERATION_THRESHOLD} قبل التقييد المؤقت.`); } return { id: await createForumTopic({ ...input, authorUserId: ctx.user.id }), status: "pending" as const }; }),
    createReply: protectedProcedure.input(z.object({ topicId: z.number().int().positive(), body: z.string().trim().min(5, "الرد قصير جداً").max(5000) })).mutation(async ({ input, ctx }) => { if (!isForumOpenAt()) throw new Error(FORUM_CLOSED_MESSAGE); if (!(await hasAcceptedForumRules(ctx.user.id))) throw new Error("يجب الموافقة على قواعد المنتدى قبل المشاركة"); const status = await getForumModerationStatus(ctx.user.id); if (status.isBlocked) throw new Error(`تم إيقاف المشاركة مؤقتاً. يمكنك المحاولة بعد نحو ${Math.max(1, Math.ceil(status.remainingMs / 60000))} دقيقة.`); const sensitive = findSensitiveForumData(input.body); if (sensitive) { await recordForumModerationViolation(ctx.user.id, new Date(), { sourceType: "reply", sourceId: input.topicId, category: sensitive.category, text: input.body, redactedExcerpt: redactSensitiveForumData(input.body) }); throw new Error(FORUM_SENSITIVE_DATA_WARNING); } const blockedTerm = await findActiveForumBlockedTerm(input.body); if (blockedTerm) { const violation = await recordForumModerationViolation(ctx.user.id, new Date(), { sourceType: "reply", sourceId: input.topicId, text: input.body, matchedTerm: blockedTerm }); if (violation.isBlocked) throw new Error(`تكرر نشر محتوى مخالف؛ تم إيقاف المشاركة مؤقتاً لمدة ${Math.ceil(violation.remainingMs / 60000)} دقيقة.`); throw new Error(`${FORUM_BLOCKED_WORD_WARNING} المحاولة ${violation.violationCount} من ${FORUM_MODERATION_THRESHOLD} قبل التقييد المؤقت.`); } return { id: await createForumReply({ ...input, authorUserId: ctx.user.id }), status: "pending" as const }; }),
  ...
  admin: router({
    testNotification: adminProcedure.mutation(async ({ ctx }) => {
      const result = await createDemoAdminNotification();
      await createAuditLog({ actorUserId: ctx.user.id, action: "notification.demo.create", entityType: "admin_notification", entityId: String(result.id), productCode: DEFAULT_PRODUCT_CODE, metadataJson: JSON.stringify({ created: result.created }) });
      return { ...result, message: result.created ? "تم إنشاء التنبيه التجريبي" : "التنبيه التجريبي موجود مسبقاً" };
    }),
    deleteDemoNotifications: adminProcedure.mutation(async ({ ctx }) => {
      const result = await deleteDemoAdminNotification();
      await createAuditLog({ actorUserId: ctx.user.id, action: "notification.demo.delete", entityType: "admin_notification", entityId: "demo-notification", productCode: DEFAULT_PRODUCT_CODE, metadataJson: JSON.stringify(result) });
      return { ...result, message: result.deleted > 0 ? "تم حذف التنبيه التجريبي" : "لا يوجد تنبيه تجريبي للحذف" };
    }),
    forumModerators: ownerProcedure.query(async () => getForumModerators(true)),
    grantForumModerator: ownerProcedure.input(z.object({ userId: z.number().int().positive(), reason: z.string().trim().max(500).optional() })).mutation(async ({ input, ctx }) => { const moderator = await grantForumModerator(input.userId, ctx.user.id); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.moderator.grant", entityType: "forum_moderator", entityId: String(input.userId), productCode: DEFAULT_PRODUCT_CODE, metadataJson: JSON.stringify({ reason: input.reason ?? null }) }); return moderator; }),
    revokeForumModerator: ownerProcedure.input(z.object({ userId: z.number().int().positive(), reason: z.string().trim().max(500).optional() })).mutation(async ({ input, ctx }) => { const moderator = await revokeForumModerator(input.userId, ctx.user.id); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.moderator.revoke", entityType: "forum_moderator", entityId: String(input.userId), productCode: DEFAULT_PRODUCT_CODE, metadataJson: JSON.stringify({ reason: input.reason ?? null }) }); return moderator; }),
    forumQueue: forumModeratorProcedure.input(z.object({ status: z.enum(["pending", "published", "hidden", "closed"]).optional(), search: z.string().trim().max(160).optional(), itemType: z.enum(["topic", "reply"]).optional(), level: z.array(FORUM_LEVEL_SCHEMA).max(FORUM_LEVELS.length).optional() }).optional()).query(async ({ input }) => getForumModerationQueue(input ?? {})),
    forumReports: forumModeratorProcedure.query(async () => getOpenForumReports()),
    moderateTopic: forumModeratorProcedure.input(z.object({ id: z.number().int().positive(), status: z.enum(["pending", "published", "hidden", "closed"]), reason: z.string().trim().max(500).optional() })).mutation(async ({ input, ctx }) => { await updateForumTopicStatus(input.id, input.status); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.topic.moderate", entityType: "forum_topic", entityId: String(input.id), productCode: DEFAULT_PRODUCT_CODE, metadataJson: JSON.stringify({ status: input.status, reason: input.reason ?? null }) }); return { success: true as const }; }),
    moderateReply: forumModeratorProcedure.input(z.object({ id: z.number().int().positive(), status: z.enum(["pending", "published", "hidden"]), reason: z.string().trim().max(500).optional() })).mutation(async ({ input, ctx }) => { await updateForumReplyStatus(input.id, input.status); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.reply.moderate", entityType: "forum_reply", entityId: String(input.id), productCode: DEFAULT_PRODUCT_CODE, metadataJson: JSON.stringify({ status: input.status, reason: input.reason ?? null }) }); return { success: true as const }; }),
    reviewForumReport: forumModeratorProcedure.input(z.object({ id: z.number().int().positive(), status: z.enum(["reviewed", "dismissed"]) })).mutation(async ({ input, ctx }) => { await updateForumReportStatus(input.id, input.status, ctx.user.id); return { success: true as const }; }),
    blockedWords: forumModeratorProcedure.query(async () => getForumBlockedWords(true)),
    addBlockedWord: forumModeratorProcedure.input(z.object({ term: z.string().trim().min(2).max(120) })).mutation(async ({ input, ctx }) => { const id = await createForumBlockedWord({ term: input.term, createdByUserId: ctx.user.id }); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.blocked_word.create", entityType: "forum_blocked_word", entityId: String(id), productCode: DEFAULT_PRODUCT_CODE, metadataJson: JSON.stringify({ term: input.term }) }); return { id }; }),
    setBlockedWordActive: forumModeratorProcedure.input(z.object({ id: z.number().int().positive(), isActive: z.boolean() })).mutation(async ({ input, ctx }) => { await updateForumBlockedWord(input.id, { isActive: input.isActive }); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.blocked_word.toggle", entityType: "forum_blocked_word", entityId: String(input.id), productCode: DEFAULT_PRODUCT_CODE, metadataJson: JSON.stringify({ isActive: input.isActive }) }); return { success: true as const }; }),
    deleteBlockedWord: forumModeratorProcedure.input(z.object({ id: z.number().int().positive() })).mutation(async ({ input, ctx }) => { await deleteForumBlockedWord(input.id); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.blocked_word.delete", entityType: "forum_blocked_word", entityId: String(input.id), productCode: DEFAULT_PRODUCT_CODE }); return { success: true as const }; }),
    forumModerationBlocks: forumModeratorProcedure.query(async () => getActiveForumModerations()),
    forumViolationMonitoring: forumModeratorProcedure.input(z.object({ search: z.string().trim().max(160).optional(), includeResolved: z.boolean().default(false) }).optional()).query(async ({ input }) => getForumViolationMonitoring(input ?? {})),
    forumParticipantClassification: forumModeratorProcedure.input(z.object({ search: z.string().trim().max(160).optional() }).optional()).query(async ({ input }) => getForumParticipantClassification(input ?? {})),
    forumWeeklyViolationReport: forumModeratorProcedure.query(async () => getForumWeeklyViolationReport()),
    clearForumModerationBlock: forumModeratorProcedure.input(z.object({ userId: z.number().int().positive() })).mutation(async ({ input, ctx }) => { await clearForumModerationBlock(input.userId); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.user_moderation.clear_block", entityType: "forum_user_moderation", entityId: String(input.userId), productCode: DEFAULT_PRODUCT_CODE }); return { success: true as const }; }),
    resetForumViolationCounter: forumModeratorProcedure.input(z.object({ userId: z.number().int().positive() })).mutation(async ({ input, ctx }) => { await resetForumViolationCounter(input.userId); await createAuditLog({ actorUserId: ctx.user.id, action: "forum.user_moderation.reset_counter", entityType: "forum_user_moderation", entityId: String(input.userId), productCode: DEFAULT_PRODUCT_CODE }); return { success: true as const }; }),
    ...
    markAllNotificationsRead: adminProcedure.mutation(async () => markAllAdminNotificationsRead()),
