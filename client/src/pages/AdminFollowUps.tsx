...
function AdminFollowUpsContent() {
  ...
  return <section dir="rtl" className="mx-auto max-w-7xl space-y-6">
    ...
    <div className="overflow-hidden rounded-[24px] border border-[#e3d9ca] bg-white shadow-sm"><div className="overflow-x-auto"><table className="min-w-[760px] w-full text-right text-sm"><thead className="bg-[#f8f3eb] text-[#173247]"><tr><th className="px-4 py-4 font-bold">التاريخ</th><th className="px-4 py-4 font-bold">بيانات التواصل</th><th className="px-4 py-4 font-bold">الرسالة</th><th className="px-4 py-4 font-bold">الحالة</th><th className="px-4 py-4 font-bold">الإجراء</th></tr></thead><tbody>{rows.map(row => { const meta = statusMeta(row.status); return <tr key={row.id} className="border-t border-[#eee7dc] align-top"><td className="px-4 py-4 text-xs text-[#68747a]">{formatDate(row.createdAt)}</td><td className="px-4 py-4 text-xs text-[#173247]"><div className="space-y-1" dir="ltr">{row.phone && <p className="font-mono">{row.phone}</p>}{row.email && <p className="break-all text-[#68747a]">{row.email}</p>}{!row.phone && !row.email && <p>—</p>}</div></td><td className="max-w-[360px] whitespace-pre-wrap px-4 py-4 leading-6 text-[#173247]">{row.message || "—"}</td><td className="px-4 py-4"><span className={`rounded-full px-3 py-1 text-xs font-bold ${meta.className}`}>{meta.label}</span></td><td className="px-4 py-4"><button type="button" onClick={() => openDetail(row.id)} className="rounded-xl border border-[#b9854a] px-3 py-2 text-xs font-bold text-[#89663b] hover:bg-[#fffaf1]">مراجعة</button></td></tr>; })}</tbody></table>{!listQuery.isLoading && !rows.length && <p className="px-4 py-12 text-center text-sm text-[#68747a]">لا توجد طلبات متابعة مطابقة.</p>}</div></div>
