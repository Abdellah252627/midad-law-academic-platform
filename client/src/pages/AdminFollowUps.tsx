...
import { Loader2, MessageCircle, Search, ShieldAlert, X } from "lucide-react";
...
function AdminFollowUpsContent() {
  ...
  return <section dir="rtl" className="mx-auto max-w-7xl space-y-6">
    <header className="rounded-[28px] bg-[#173247] p-6 text-white shadow-[0_20px_60px_rgba(23,50,71,0.16)] sm:p-8">
      ...
      <p className="mt-2 max-w-2xl text-sm leading-7 text-white/70">راجع الرسائل وأرقام الهاتف التي تركها الزوار خارج أوقات العمل، ثم حدّث الحالة بعد التواصل معهم. تُعرض هذه البيانات داخل Back Office فقط.</p>
    </header>
    ...
    <form onSubmit={event => { event.preventDefault(); setSearch(searchDraft.trim().slice(0, 160)); setPage(1); }} className="flex flex-col gap-3 rounded-[22px] border border-[#e3d9ca] bg-white p-4 shadow-sm lg:flex-row lg:items-center">
      <div className="relative flex-1"><Search className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#9b8f80]" aria-hidden="true" /><input value={searchDraft} onChange={event => setSearchDraft(event.target.value)} placeholder="ابحث في الرسالة أو رقم الهاتف" aria-label="البحث في طلبات التواصل" maxLength={160} className="w-full rounded-xl border border-[#e3d9ca] bg-[#fcfaf6] py-3 pr-10 pl-4 text-sm text-[#173247] outline-none focus:border-[#b9854a] focus:ring-2 focus:ring-[#b9854a]/20" /></div>
    ...
    <div className="overflow-hidden rounded-[24px] border border-[#e3d9ca] bg-white shadow-sm"><div className="overflow-x-auto"><table className="min-w-[760px] w-full text-right text-sm"><thead className="bg-[#f8f3eb] text-[#173247]"><tr><th className="px-4 py-4 font-bold">التاريخ</th><th className="px-4 py-4 font-bold">رقم الهاتف</th><th className="px-4 py-4 font-bold">الرسالة</th><th className="px-4 py-4 font-bold">الحالة</th><th className="px-4 py-4 font-bold">الإجراء</th></tr></thead><tbody>{rows.map(row => { const meta = statusMeta(row.status); return <tr key={row.id} className="border-t border-[#eee7dc] align-top"><td className="px-4 py-4 text-xs text-[#68747a]">{formatDate(row.createdAt)}</td><td className="px-4 py-4 font-mono text-xs text-[#173247]" dir="ltr">{row.phone || "—"}</td><td className="max-w-[360px] whitespace-pre-wrap px-4 py-4 leading-6 text-[#173247]">{row.message || "—"}</td><td className="px-4 py-4"><span className={`rounded-full px-3 py-1 text-xs font-bold ${meta.className}`}>{meta.label}</span></td><td className="px-4 py-4"><button type="button" onClick={() => openDetail(row.id)} className="rounded-xl border border-[#b9854a] px-3 py-2 text-xs font-bold text-[#89663b] hover:bg-[#fffaf1]">مراجعة</button></td></tr>; })}</tbody></table>{!listQuery.isLoading && !rows.length && <p className="px-4 py-12 text-center text-sm text-[#68747a]">لا توجد طلبات متابعة مطابقة.</p>}</div></div>
