...
import { FAQ_CATEGORY_DESCRIPTIONS, FAQ_CATEGORY_LABELS, FAQ_CATEGORIES, type FaqCategory } from "@shared/faq";
...
const fallbackFaqs: Array<[string, string, FaqCategory]> = [
  ["ماذا يشمل الشراء؟", "يشمل الوصول إلى الملف الرقمي الخاص بالمنتج، والاستفادة من التحديثات والفصول القادمة المرتبطة به مجاناً مدى الحياة، بغض النظر عن السعر الذي تم دفعه.", "purchase"],
  ["كيف يتم تأكيد الطلب وتسليم الملف؟", "ترسل بيانات الطلب وإثبات التحويل البنكي، ثم تراجع الإدارة العملية يدوياً. بعد اعتماد الطلب، يُتاح لك رابط تنزيل خاص وفق آلية التسليم المعتمدة في المنصة.", "purchase"],
  ["هل يمكن طلب استرداد المبلغ بعد تنزيل الملف؟", "تُراجع طلبات الاسترداد وفق حالة الطلب واستخدام رابط التنزيل وسياسة الملفات الرقمية المنشورة. تواصل مع الإدارة برقم الطلب قبل اتخاذ أي إجراء لضمان مراجعة الحالة بدقة.", "purchase"],
  ...
  ["كم تستغرق مراجعة الطلب؟", "تتم مراجعة التحويل البنكي يدوياً عادةً خلال 24 ساعة من وصول بيانات العملية وإثبات الدفع، وقد تمتد المدة قليلاً في العطل أو عند الحاجة إلى تحقق إضافي. إذا تأخرت الحالة، أرسل رقم الطلب والبريد أو رقم واتساب المستخدم عبر صفحة الدعم أو واتساب لتسهيل المتابعة.", "payment"],
  ["ماذا أفعل إذا أرسلت بيانات تحويل غير صحيحة؟", "تواصل سريعاً مع مركز الخدمات عبر واتساب، واذكر رقم الطلب ومرجع العملية والبيانات الصحيحة. لا ترسل عملية جديدة قبل تلقي توجيه واضح من الإدارة.", "payment"],
  ["ما طبيعة منتج MIDAD؟", "MIDAD ملخص رقمي منظم يساعد طالب القانون على بناء أساس واضح للمراجعة وفهم المفاهيم الأساسية، مع أسئلة قصيرة لاختبار الفهم. وهو مادة مساعدة، وليس بديلاً عن المحاضرات أو المراجع التي يحددها الأستاذ.", "content"],
  ["هل هذا ملخص رسمي صادر عن جامعة ابن زهر؟", "لا. هذا منتج تعليمي رقمي مستقل موجّه إلى الطلبة للمراجعة الذاتية. لا يمثل مقرراً رسمياً أو وثيقة صادرة عن الجامعة، ولا يعني أن الجامعة راجعت المنتج أو اعتمدته.", "content"],
  ["ماذا تعني المراجعة التطبيقية؟", "تعني أن المحتوى لا يكتفي بعرض المفاهيم، بل يساعدك على ترتيبها وفهم صلتها ببعضها واسترجاعها من خلال أمثلة وأسئلة قصيرة واختبارات فهم عند توفرها. وهي أداة للمراجعة الذاتية، وليست تصحيحاً رسمياً لأجوبة الامتحان أو بديلاً عن المحاضرات والمراجع.", "content"],
  ["ماذا تعني التحديثات المجانية مدى الحياة؟", "يعني ذلك أن من يشتري المنتج يحصل على التحديثات والفصول القادمة المرتبطة به دون دفع رسوم إضافية بسبب إصدارها لاحقاً، بغض النظر عن السعر الذي دفعه وقت الشراء. ولا يشمل ذلك منتجات مستقلة جديدة أو خدمات استشارية أو دروساً فردية غير معلنة ضمن المنتج.", "content"],
  ["هل يضمن الملخص النجاح في الامتحان؟", "لا يمكن لأي ملخص أن يضمن نتيجة امتحان. صُمّم المنتج للمساعدة في الفهم والتنظيم والمراجعة، وتبقى النتيجة مرتبطة بانتظام الطالب وقراءته للمحاضرات والمراجع والتزامه بتوجيهات الأستاذ.", "content"],
  ["ماذا أفعل إذا لم يظهر رابط التنزيل أو انتهت صلاحيته؟", "تحقق أولاً من البريد المستخدم في الطلب، ثم تواصل عبر صفحة الدعم أو واتساب مع ذكر رقم الطلب. تراجع الإدارة حالة الطلب وتصدر رابطاً صالحاً وفق سجل التسليم.", "support"],
  ["ماذا أفعل إذا أخطأت في البريد أو رقم واتساب؟", "استخدم طلب تصحيح البيانات المرتبط برقم الطلب، أو تواصل عبر واتساب مع ذكر رقم الطلب بوضوح. تراجع الإدارة التصحيح قبل تطبيقه حفاظاً على سلامة التسليم.", "support"],
  ["هل يمكنني قراءة الملف على الهاتف؟", "نعم. الملف بصيغة PDF ومهيأ للقراءة الرقمية بالعربية، مع تنسيق مناسب للهاتف والحاسوب والطباعة الشخصية.", "support"],
  ["هل يشمل المنتج استشارة قانونية؟", "لا. المحتوى تعليمي عام للمراجعة، ولا يشكل استشارة قانونية أو رأياً مهنياً في واقعة محددة.", "support"],
];
...
export default function Home() {
  ...
  const faqs = (landingQuery.data?.faqs?.length
    ? landingQuery.data.faqs.map(faq => ({ question: faq.question, answer: faq.answer, category: (faq.category ?? "support") as FaqCategory }))
    : fallbackFaqs.map(([question, answer, category]) => ({ question, answer, category })).map(faq => {
      const isTransferFaq = faq.question.includes("التحويل") || faq.question.includes("الدفع") || faq.question.includes("مراجعة الطلب");
      return { ...faq, answer: isTransferFaq ? faq.answer.replace(/خلال\s+\d+\s+ساعة/g, `خلال ${bankTransferReviewDurationLabel}`) : faq.answer };
    }));

  const [menuOpen, setMenuOpen] = useState(false);
  const [activeFaqCategory, setActiveFaqCategory] = useState<FaqCategory>("purchase");
  ...
  const [followUpForm, setFollowUpForm] = useState({ phone: "", email: "", message: "" });
  const [followUpSent, setFollowUpSent] = useState(false);
  const [followUpReference, setFollowUpReference] = useState("");
  const submitSupportFollowUp = trpc.support.submitFollowUp.useMutation({
    onSuccess: result => {
      setFollowUpSent(true);
      setFollowUpReference(result.reference);
      setFollowUpForm({ phone: "", email: "", message: "" });
      toast.success("تم استلام طلبك، وسيتواصل معك الفريق عند بدء ساعات العمل.");
    },
    onError: error => toast.error(error.message || "تعذر إرسال طلب المتابعة."),
  });
    ...
    <div dir="rtl" className="min-h-screen overflow-x-hidden bg-[#f7f3eb] text-[#172b3a] selection:bg-[#b9854a]/30">
      ...
      <main id="top">
        <section className="relative isolate min-h-[720px] overflow-hidden border-b border-[#e5ded2] pt-[74px]">
          ...
          <div className="mx-auto grid max-w-[1240px] items-center gap-14 px-5 py-20 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20 lg:px-8 lg:py-28">
            <div className="order-2 max-w-[650px] lg:order-1">
              ...
              <div className="welcome-message mt-6 max-w-[610px] rounded-2xl border border-[#d8c6aa] bg-[#fffaf1]/80 px-5 py-4 shadow-[0_12px_30px_rgba(185,133,74,0.08)]" role="note" aria-label="رسالة ترحيبية لطلبة القانون في المغرب">
        ...
        <section id="why" className="border-b border-[#e5ded2] bg-[#172b3a] text-[#f7f3eb]">
          ...
          <div className="mx-auto max-w-[1240px] px-5 pb-12 lg:px-8">
            <a href="/forum" className="inline-flex items-center gap-2 rounded-full border border-[#cfa56f]/70 px-5 py-3 text-sm font-extrabold text-[#f7f3eb] transition hover:border-[#d5a15f] hover:bg-[#cfa56f]/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d5a15f]">
              اكتشف منتدى Midad Law <ArrowLeft size={16} aria-hidden="true" />
            </a>
          </div>
        </section>
        ...
        <section id="faq" className="bg-[#f7f3eb] py-24 lg:py-32">
          <div className="mx-auto max-w-[900px] px-5 lg:px-8"><div className="text-center"><div className="section-kicker justify-center">05 / الأسئلة الشائعة</div><h2 className="mt-4 font-display text-4xl font-black tracking-[-0.05em] text-[#172b3a] md:text-5xl">إجابات واضحة قبل الطلب</h2><p className="mx-auto mt-5 max-w-2xl text-sm leading-[2] text-[#68747a]">اقرأ التفاصيل الأساسية حول طبيعة الملخص، طريقة الطلب، التسليم، والتحديثات المجانية قبل اتخاذ قرارك.</p></div><div className="mt-10 flex flex-wrap justify-center gap-2" role="tablist" aria-label="تصنيفات الأسئلة الشائعة">{FAQ_CATEGORIES.map(category => <button key={category} type="button" role="tab" aria-selected={activeFaqCategory === category} onClick={() => { setActiveFaqCategory(category); setOpenFaq(0); }} className={`rounded-full border px-4 py-2 text-xs font-black transition ${activeFaqCategory === category ? "border-[#173247] bg-[#173247] text-white" : "border-[#d9d0c2] bg-white/60 text-[#68747a] hover:border-[#b9854a] hover:text-[#89663b]"}`}>{FAQ_CATEGORY_LABELS[category]}</button>)}</div><p className="mt-3 text-center text-xs text-[#8b7357]">{FAQ_CATEGORY_DESCRIPTIONS[activeFaqCategory]}</p><div className="mt-8 divide-y divide-[#d9d0c2] border-y border-[#d9d0c2]">{faqs.filter(faq => faq.category === activeFaqCategory).map((faq, index) => <div key={faq.question}><button onClick={() => setOpenFaq(openFaq === index ? null : index)} className="flex w-full items-center justify-between gap-5 py-6 text-right font-display text-[15px] font-extrabold text-[#172b3a] transition hover:text-[#b9854a]" aria-expanded={openFaq === index}><span>{faq.question}</span><ChevronDown size={18} className={`shrink-0 text-[#b9854a] transition-transform duration-200 ${openFaq === index ? "rotate-180" : ""}`} /></button><div className={`grid transition-[grid-template-rows,opacity] duration-200 ${openFaq === index ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"}`}><div className="overflow-hidden"><p className="max-w-[760px] pb-6 font-body text-[14px] leading-[2] text-[#68747a]">{faq.answer}</p></div></div></div>)}{faqs.filter(faq => faq.category === activeFaqCategory).length === 0 && <p className="py-8 text-center text-sm text-[#68747a]">لا توجد أسئلة منشورة ضمن هذه الفئة حالياً.</p>}</div><div className="mt-10 flex justify-center"><a href="/forum" className="inline-flex items-center gap-2 rounded-full bg-[#172b3a] px-6 py-3 text-sm font-extrabold text-white transition hover:bg-[#b9854a] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#172b3a]">تحدث مع زملائك في المنتدى <ArrowLeft size={16} aria-hidden="true" /></a></div></div>
        </section>
