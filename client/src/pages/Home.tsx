      ...
      <main id="top">
        ...
        <section id="purchase" className="relative overflow-hidden bg-[#172b3a] py-24 text-[#f7f3eb] lg:py-32">
          ...
          <div className="relative mx-auto grid max-w-[1100px] gap-12 px-5 lg:grid-cols-[1fr_380px] lg:items-center lg:px-8">
            ...
            <div className="relative rounded-[24px] border border-white/15 bg-[#f7f3eb] p-7 text-[#172b3a] shadow-[0_25px_70px_rgba(0,0,0,0.22)] sm:p-9"><div className="flex items-start justify-between"><div><span className="font-display text-[11px] font-black tracking-[0.15em] text-[#b9854a]">{DEFAULT_PRODUCT_CODE}</span><h3 className="mt-3 font-display text-xl font-black leading-[1.4]">مدخل إلى القانون<br />والعلوم القانونية</h3></div><div className="rounded-full bg-[#e8eee9] p-3 text-[#b9854a]"><BookOpen size={19} /></div></div><div className="my-7 border-t border-[#ded6c9]" /><div className="flex items-end justify-between gap-4"><div><span className="font-body text-xs text-[#768087]">{earlyBirdActive ? "سعر Early Bird" : "السعر الحالي"}</span><div className="mt-1 flex items-baseline gap-2"><span className="font-display text-5xl font-black text-[#172b3a]">{productPriceMad}</span><span className="font-display text-sm font-black text-[#b9854a]">درهماً</span></div></div><span className="rounded-full bg-[#b9854a]/10 px-3 py-2 text-center text-[10px] font-extrabold leading-[1.6] text-[#89663b]">{earlyBirdActive ? `متبقّي ${earlyBirdSeatsRemaining} مقاعد` : "السعر الدائم"}</span></div><button onClick={handleCheckout} className="mt-8 flex w-full items-center justify-center gap-3 rounded-full bg-[#b9854a] px-5 py-4 text-sm font-extrabold text-white transition hover:bg-[#a8733d] hover:shadow-[0_12px_25px_rgba(185,133,74,0.3)] active:scale-[0.98]">احصل على الملخص الآن <ArrowLeft size={17} /></button>{earlyBirdActive && <p className="mt-4 text-center text-xs font-bold leading-[1.8] text-[#89663b]">سعر خاص لأول 10 مشترين مقبولين — تبقّت {earlyBirdSeatsRemaining} مقاعد.</p>}<p className="mt-3 text-center font-body text-[11px] leading-[1.8] text-[#68747a]">جميع المشترين يحصلون على التحديثات والفصول القادمة مجاناً مدى الحياة، بغض النظر عن وقت الشراء أو السعر.</p><p className="mt-2 text-center font-body text-[11px] leading-[1.8] text-[#8b9290]">الدفع والتسليم الإلكتروني قيد الربط في الإصدار التالي.</p></div>
          </div>
        </section>
