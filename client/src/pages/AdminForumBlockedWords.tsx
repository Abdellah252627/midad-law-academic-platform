...
import { Clock3, Loader2, Plus, ShieldCheck, Trash2, Unlock, UserRound } from "lucide-react";
...
function BlockedWordsContent() {
  ...
  const blocks = trpc.admin.forumModerationBlocks.useQuery();
  const clearBlock = trpc.admin.clearForumModerationBlock.useMutation({ onSuccess: () => { toast.success("تم فك الحظر المؤقت وإعادة عداد المخالفات"); void utils.admin.forumModerationBlocks.invalidate(); }, onError: error => toast.error(error.message) });
  ...
  return <div dir="rtl" className="mx-auto max-w-5xl space-y-6">
    ...
    <section className="rounded-[26px] border border-[#e3d9ca] bg-white p-5 shadow-sm sm:p-7">
      <div className="flex items-start justify-between gap-4"><div><h2 className="flex items-center gap-2 text-xl font-bold text-[#173247]"><UserRound className="h-5 w-5 text-[#b9854a]" />الحظر المؤقت النشط</h2><p className="mt-2 text-sm leading-7 text-slate-500">يظهر هنا المستخدمون الذين تجاوزوا ثلاث محاولات مخالفة خلال نافذة 24 ساعة. يبدأ الحظر من 30 دقيقة ويتدرج تلقائياً عند التكرار.</p></div><span className="rounded-full bg-red-50 px-3 py-1 text-xs font-bold text-red-700">{activeBlocks.length} محظور</span></div>
      {blocks.isLoading ? <div className="flex items-center gap-2 py-10 text-slate-500"><Loader2 className="h-5 w-5 animate-spin" />جاري تحميل حالات الحظر...</div> : activeBlocks.length === 0 ? <div className="mt-5 rounded-2xl border border-dashed border-slate-200 bg-[#fffdf9] p-6 text-center text-sm text-slate-500">لا توجد حالات حظر مؤقت نشطة حالياً.</div> : <div className="mt-5 space-y-3">{activeBlocks.map(({ moderation, user }) => <article key={moderation.id} className="flex flex-col gap-4 rounded-2xl border border-red-100 bg-red-50/40 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-bold text-[#173247]">{user.name || "مستخدم المنتدى"}</p><p className="mt-1 text-xs text-slate-500">{user.email || "بريد غير متاح"} · {moderation.violationCount} محاولات مخالفة · مستوى {moderation.blockLevel}</p><p className="mt-2 flex items-center gap-1 text-xs font-bold text-red-700"><Clock3 className="h-4 w-4" />ينتهي في {moderation.blockedUntil ? new Date(moderation.blockedUntil).toLocaleString("ar-MA") : "غير محدد"}</p></div><button type="button" onClick={() => { if (window.confirm("هل تريد فك الحظر وإعادة عداد المخالفات لهذا المستخدم؟")) clearBlock.mutate({ userId: user.id }); }} disabled={clearBlock.isPending} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-[#b9854a]/30 bg-white px-4 text-sm font-bold text-[#173247] transition hover:bg-[#fff7ec] disabled:opacity-50"><Unlock className="h-4 w-4" />فك الحظر</button></article>)}</div>}
    </section>
  </div>;
}
