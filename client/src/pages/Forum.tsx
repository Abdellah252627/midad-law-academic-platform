...
import { ArrowRight, Flag, MessageCircle, ShieldCheck } from "lucide-react";
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        ...
        {selectedTopicId && <Card className="border-[#d6a15b]/50 bg-white/95 shadow-md"><CardHeader><div className="flex items-center justify-between gap-3"><CardTitle>{topicDetail.data?.topic.title ?? "تفاصيل الموضوع"}</CardTitle><Button variant="ghost" onClick={() => setSelectedTopicId(undefined)}>إغلاق</Button></div></CardHeader><CardContent className="space-y-6">
          {topicDetail.isLoading ? <p className="text-sm text-[#59636a]">جارٍ تحميل الموضوع...</p> : topicDetail.data ? <>
            ...
            {user && hasAcceptedRules && <form className="space-y-3 border-t border-[#e1d4bf] pt-5" onSubmit={event => { event.preventDefault(); createReply.mutate({ topicId: selectedTopicId, body: replyBody }); }}><h3 className="font-bold">أضف رداً تعليمياً</h3>{moderationStatus.data?.showWarning && <p role="status" className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm font-bold leading-6 text-amber-900"><ShieldCheck className="ml-1 inline h-4 w-4" />{moderationStatus.data.message}</p>}<Textarea value={replyBody} onChange={event => setReplyBody(event.target.value)} placeholder="اكتب رداً محترماً ومفيداً..." maxLength={5000} /><Button disabled={createReply.isPending || moderationStatus.data?.isBlocked || replyBody.trim().length < 5}>{moderationStatus.data?.isBlocked ? `المشاركة متوقفة مؤقتاً (${moderationStatus.data.remainingMinutes} د)` : createReply.isPending ? "جارٍ الإرسال للمراجعة..." : "إرسال الرد للمراجعة"}</Button></form>}
