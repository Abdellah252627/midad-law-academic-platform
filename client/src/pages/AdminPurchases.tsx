...
<div className="grid grid-cols-2 gap-3 sm:gap-4">
      {[['الإجمالي', requests.length], ['قيد المراجعة', requests.filter(item => item.status === 'pending').length], ['المقبولة', requests.filter(item => item.status === 'approved').length], ['المرفوضة', requests.filter(item => item.status === 'rejected').length]].map(([label, value]) => <div key={label as string} className="rounded-[22px] border border-[#e3d9ca] bg-white p-5 shadow-sm"><p className="text-sm font-bold text-[#68747a]">{label as string}</p><p className="mt-2 text-3xl font-bold text-[#173247]">{requestsQuery.isLoading ? "…" : value as number}</p></div>)}
    </div>
