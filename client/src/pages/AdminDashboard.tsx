...
import { FAQ_CATEGORY_LABELS, FAQ_CATEGORIES, type FaqCategory } from "@shared/faq";
...
import { BarChart3, Check, Eye, FileText, Headset, HelpCircle, ListChecks, Loader2, Plus, RefreshCw, RotateCcw, Save, Settings2, ShoppingBag, Trash2, Users, X } from "lucide-react";
import type { LucideIcon } from "lucide-react";
import { FormEvent, useEffect, useMemo, useState } from "react";
import { toast } from "sonner";

type Tab = "product" | "chapters" | "upcoming" | "quiz" | "faqs";
...
const emptyFaq = { question: "", answer: "", category: "support" as FaqCategory, sortOrder: 0, isPublished: 1 as 0 | 1 };
const blankQuestion = (): QuizEditorQuestion => ({ question: "", options: ["", "", "", ""], correctIndex: 0, explanation: "", reviewConcept: "" });
const defaultUpcomingChapters = ["قانون الالتزامات والعقود", "القانون الدستوري", "التنظيم الإداري", "القانون الجنائي العام", "المدخل إلى العلوم السياسية", "المدخل إلى علم الاقتصاد", "القانون المدني", "القانون التجاري", "قانون الشغل"];
...
function AdminDashboardContent() {
  ...
  const [faqPreviewCategory, setFaqPreviewCategory] = useState<FaqCategory | null>(null);
  const [upcomingChapters, setUpcomingChapters] = useState(defaultUpcomingChapters.join("\n"));
  ...
  const settingsQuery = trpc.admin.settings.useQuery(contentInput);
  const analyticsQuery = trpc.admin.analyticsSummary.useQuery();
  ...
  const saveUpcoming = trpc.admin.saveSetting.useMutation({ onSuccess: () => { toast.success("تم حفظ الفصول القادمة"); void utils.admin.settings.invalidate(contentInput); void utils.landing.published.invalidate({ productCode }); }, onError: error => toast.error(error.message) });
  ...
  useEffect(() => { const saved = settingsQuery.data?.find(item => item.settingKey === "upcomingChapters")?.settingValue; if (saved) { try { const parsed = JSON.parse(saved); if (Array.isArray(parsed)) setUpcomingChapters(parsed.map(String).join("\n")); } catch { /* keep safe fallback */ } } }, [settingsQuery.data]);
  const chapters = contentQuery.data?.chapters ?? [];
  const faqs = contentQuery.data?.faqs ?? [];
  const faqCategoryCounts = FAQ_CATEGORIES.map(category => ({ category, count: faqs.filter(item => (item.category ?? "support") === category).length }));
  const faqPreviewItems = useMemo(() => {
    if (!faqPreviewCategory) return [];
    const savedItems = faqs
      .filter(item => (item.category ?? "support") === faqPreviewCategory)
      .map(item => ({ id: item.id, question: item.question, answer: item.answer, isDraft: false }));
    if (!faq.question.trim() || !faq.answer.trim() || faq.category !== faqPreviewCategory) return savedItems;
    const draft = { id: faqId ?? -1, question: faq.question.trim(), answer: faq.answer.trim(), isDraft: true };
    const existingIndex = savedItems.findIndex(item => item.id === faqId);
    if (existingIndex >= 0) return savedItems.map((item, index) => index === existingIndex ? draft : item);
    return [...savedItems, draft];
  }, [faq, faqId, faqPreviewCategory, faqs]);
  const faqPreviewLabel = faqPreviewCategory ? FAQ_CATEGORY_LABELS[faqPreviewCategory] : "";
  ...
  const handleUpcoming = (event: FormEvent) => { event.preventDefault(); const titles = upcomingChapters.split("\n").map(item => item.trim()).filter(Boolean); if (!titles.length || titles.length > 30) { toast.error("أدخل من فصلاً واحداً إلى 30 فصلاً، كل فصل في سطر مستقل"); return; } saveUpcoming.mutate({ productCode, settingKey: "upcomingChapters", settingValue: JSON.stringify(titles), description: "عناوين الفصول القادمة المعروضة للزوار" }); };
  ...
  return <div dir="rtl" className="mx-auto max-w-7xl space-y-6">
    <header className="rounded-[28px] bg-[#173247] p-6 text-white shadow-[0_20px_60px_rgba(23,50,71,0.16)] sm:p-8"><p className="text-xs font-bold tracking-[0.18em] text-[#d5a15f]">MIDAD / CONTROL CENTER</p><div className="mt-2 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h1 className="font-display text-3xl font-bold">إدارة صفحة الهبوط</h1><p className="mt-2 max-w-2xl text-sm leading-7 text-white/70">عدّل المحتوى والاختبارات من هنا ثم انشره للزوار. لا تظهر هذه الواجهة إلا لحسابات الإدارة.</p></div><div className="flex flex-wrap items-center gap-2"><Link href="/admin/follow-ups" className="inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/10 px-4 py-2 text-xs font-bold text-white transition hover:bg-white/20"><Headset className="h-4 w-4" aria-hidden="true" /><span>طلبات التواصل</span>{newFollowUpsCount > 0 && <span aria-label={`${newFollowUpsCount} طلبات تواصل جديدة`} className="inline-flex min-w-5 items-center justify-center rounded-full bg-red-500 px-1.5 py-0.5 text-[10px] font-black leading-none text-white">{newFollowUpsCount > 99 ? "99+" : newFollowUpsCount}</span>}</Link><Link href="/admin/preview" className="inline-flex items-center gap-2 rounded-full bg-[#d5a15f] px-4 py-2 text-xs font-bold text-[#173247] transition hover:bg-[#e2b875]">معاينة قبل النشر</Link><span className="rounded-full bg-white/10 px-4 py-2 text-xs font-bold">{productCode}</span></div></div></header>
    <nav className="grid gap-3 sm:grid-cols-2 md:grid-cols-5" aria-label="أقسام إدارة المحتوى">{[["product", Settings2, "بيانات المنتج"], ["chapters", FileText, "المحاور"], ["upcoming", FileText, "الفصول القادمة"], ["quiz", ListChecks, "اختبر فهمك"], ["faqs", HelpCircle, "الأسئلة الشائعة"]].map(([key, Icon, label]) => <button key={key as string} onClick={() => setTab(key as Tab)} className={`flex items-center gap-3 rounded-2xl border p-4 text-right transition ${tab === key ? "border-[#b9854a] bg-[#fffaf2] text-[#173247] shadow-sm" : "border-[#e3d9ca] bg-white text-[#68747a] hover:border-[#b9854a]/60"}`}><Icon className="h-5 w-5" /><span className="font-bold">{label as string}</span></button>)}</nav>
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {([
          ...
          [ShoppingBag, "طلبات اليوم", analyticsQuery.data?.todayPurchaseRequests ?? "…"],
          [Eye, "تحميلات العينات", analyticsQuery.data?.todaySampleDownloads ?? "…"],
          [BarChart3, "التحويل اليوم", analyticsQuery.data ? `${analyticsQuery.data.todayConversionRate}%` : "…"],
        ] as Array<[LucideIcon, string, string | number]>).map(([Icon, label, value]) => <div key={label} className="rounded-2xl border border-[#e3d9ca] bg-white p-4 shadow-sm"><div className="flex items-center gap-2 text-[#68747a]"><Icon className="h-4 w-4 text-[#b9854a]" aria-hidden="true" /><span className="text-xs font-bold">{label}</span></div><p className="mt-2 text-2xl font-black text-[#173247]">{value}</p></div>)}
      </div>
    </section>
    {contentQuery.data?.pricing && <section className="mb-5 grid gap-4 rounded-[24px] border border-[#d7c09b] bg-[#fffaf1] p-5 shadow-sm sm:grid-cols-3" aria-label="حالة تسعير Early Bird"><div><p className="text-xs font-bold text-[#68747a]">المشترون الفعليون</p><p className="mt-2 font-display text-3xl font-black text-[#173247]">{contentQuery.data.pricing.approvedBuyers} <span className="text-base font-bold text-[#68747a]">من {contentQuery.data.pricing.earlyBirdLimit}</span></p></div><div><p className="text-xs font-bold text-[#68747a]">المرحلة الحالية</p><p className="mt-2 font-display text-xl font-black text-[#173247]">{contentQuery.data.pricing.earlyBirdActive ? "Early Bird" : "السعر الدائم / اليدوي"}</p><p className="mt-1 text-xs text-[#68747a]">السعر الفعال: {contentQuery.data.pricing.priceMad} درهماً</p></div><div><p className="text-xs font-bold text-[#68747a]">المقاعد المتبقية</p><p className="mt-2 font-display text-3xl font-black text-[#b9854a]">{contentQuery.data.pricing.earlyBirdActive ? contentQuery.data.pricing.earlyBirdSeatsRemaining : "مكتملة"}</p><p className="mt-1 text-xs leading-6 text-[#68747a]">تُحسب الطلبات المقبولة فقط. السعر اليدوي لا يغيّر أسعار الطلبات السابقة.</p></div></section>}
    {contentQuery.isLoading ? <div className="rounded-2xl bg-white p-10 text-center text-[#68747a]"><Loader2 className="mx-auto h-6 w-6 animate-spin" /></div> : contentQuery.isError ? <div role="alert" className="rounded-2xl border border-red-200 bg-red-50 p-6 text-center text-sm leading-7 text-red-800"><p className="font-bold">تعذر تحميل محتوى لوحة الإدارة.</p><p className="mt-1">أعد تحميل الصفحة أو تحقق من اتصال الخادم. لم يتم حذف أي محتوى.</p><button type="button" onClick={() => void contentQuery.refetch()} className="mt-4 rounded-full bg-[#173247] px-5 py-2 text-xs font-bold text-white">إعادة المحاولة</button></div> : <>
      ...
      {tab === "upcoming" && <form onSubmit={handleUpcoming} className="space-y-5 rounded-[26px] border border-[#d7c09b] bg-[#fffaf1] p-6 shadow-sm sm:p-7"><div><p className="text-xs font-bold tracking-[0.16em] text-[#b9854a]">BACK OFFICE / UPCOMING CHAPTERS</p><h2 className="mt-2 font-display text-2xl font-black text-[#173247]">إدارة الفصول القادمة</h2><p className="mt-2 text-sm leading-7 text-[#68747a]">اكتب اسم كل فصل في سطر مستقل. تظهر هذه العناوين للزوار بوسم «قريباً» فقط، ولا تصبح قابلة للنقر أو الاختبار إلا بعد تحويلها إلى محتوى منشور.</p></div><textarea value={upcomingChapters} onChange={event => setUpcomingChapters(event.target.value)} className="min-h-72 w-full rounded-2xl border border-[#e3d9ca] bg-white px-4 py-3 leading-8 outline-none focus:border-[#b9854a]" aria-label="قائمة الفصول القادمة" /><div className="flex flex-wrap items-center justify-between gap-3"><span className="text-xs text-[#68747a]">كل سطر يمثل فصلاً واحداً، والحد الأقصى 30 فصلاً.</span><button disabled={saveUpcoming.isPending} className="inline-flex items-center gap-2 rounded-full bg-[#173247] px-5 py-3 font-bold text-white disabled:opacity-60"><Save className="h-4 w-4" />{saveUpcoming.isPending ? "جارٍ الحفظ…" : "حفظ الفصول القادمة"}</button></div></form>}
      ...
      {tab === "faqs" && <><section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4" aria-label="توزيع الأسئلة الشائعة حسب الفئة">{faqCategoryCounts.map(({ category, count }) => <div key={category} className="rounded-2xl border border-[#e3d9ca] bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-2"><p className="text-xs font-bold text-[#68747a]">{FAQ_CATEGORY_LABELS[category]}</p><span className="rounded-full bg-[#f4eadb] px-2 py-1 text-[10px] font-black text-[#89663b]">{count}</span></div><p className="mt-2 text-[11px] text-[#8b9290]">سؤال منشور أو محفوظ</p><button type="button" onClick={() => setFaqPreviewCategory(category)} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full border border-[#b9854a] bg-[#fffaf1] px-3 py-2 text-xs font-bold text-[#173247] transition hover:bg-[#f4eadb]"><Eye className="h-4 w-4" aria-hidden="true" />معاينة الفئة</button></div>)}</section><ContentManager title="الأسئلة الشائعة" emptyLabel="لا توجد أسئلة محفوظة بعد." items={faqs} onEdit={item => { setFaqId(item.id); setFaq({ question: item.question, answer: item.answer, category: (item.category ?? "support") as FaqCategory, sortOrder: item.sortOrder, isPublished: item.isPublished === 1 ? 1 : 0 }); }} onDelete={id => deleteFaq.mutate({ id })} onRestore={id => restoreFaq.mutate({ id })} renderItem={item => <span className="inline-flex items-center gap-2"><span>{item.question}</span><small className="rounded-full bg-[#f4eadb] px-2 py-1 text-[10px] font-black text-[#89663b]">{FAQ_CATEGORY_LABELS[(item.category ?? "support") as FaqCategory]}</small></span>}><form onSubmit={handleFaq} className="grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-[#d7c09b] bg-[#fffaf1] p-4 text-sm leading-7 text-[#68747a] sm:col-span-2"><p className="font-bold text-[#173247]">تحرير الأسئلة الظاهرة في صفحة الهبوط</p><p className="mt-1">أضف إجابة مختصرة ودقيقة عن طبيعة المنتج، الطلب، التحويل، التسليم، والتحديثات المجانية. فعّل «ظاهر للزوار» ثم احفظ لنشر السؤال، واستخدم الترتيب للتحكم في موضعه.</p></div><Field label="السؤال" value={faq.question} onChange={value => setFaq({ ...faq, question: value })} required /><label className="space-y-2 text-sm font-bold text-[#173247]">الفئة<select value={faq.category} onChange={event => setFaq({ ...faq, category: event.target.value as FaqCategory })} className="mt-2 w-full rounded-xl border border-[#e3d9ca] bg-[#fffdf9] px-4 py-3 outline-none focus:border-[#b9854a]">{FAQ_CATEGORIES.map(category => <option key={category} value={category}>{FAQ_CATEGORY_LABELS[category]}</option>)}</select></label><Field label="الترتيب" type="number" value={String(faq.sortOrder)} onChange={value => setFaq({ ...faq, sortOrder: Number(value) })} required /><label className="space-y-2 text-sm font-bold sm:col-span-2">الإجابة<textarea required minLength={20} value={faq.answer} onChange={event => setFaq({ ...faq, answer: event.target.value })} placeholder="اكتب إجابة واضحة من جملتين أو ثلاث، دون وعود غير قابلة للضمان…" className="mt-2 min-h-32 w-full rounded-xl border border-[#e3d9ca] bg-[#fffdf9] px-4 py-3 leading-7" /></label><label className="flex items-center gap-3 rounded-xl bg-[#f8f3eb] px-4 py-3 text-sm font-bold"><input type="checkbox" checked={faq.isPublished === 1} onChange={event => setFaq({ ...faq, isPublished: event.target.checked ? 1 : 0 })} /> ظاهر للزوار</label><button className="rounded-full bg-[#173247] px-5 py-3 font-bold text-white">{faqId ? "تحديث السؤال" : "إضافة السؤال"}</button></form></ContentManager>{faqPreviewCategory && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#173247]/55 p-4" role="dialog" aria-modal="true" aria-labelledby="faq-preview-title"><div className="max-h-[85vh] w-full max-w-3xl overflow-y-auto rounded-[26px] bg-[#fffdf9] p-6 shadow-2xl sm:p-8"><div className="flex items-start justify-between gap-4 border-b border-[#e3d9ca] pb-4"><div><p className="text-xs font-bold tracking-[0.16em] text-[#b9854a]">FAQ / PREVIEW</p><h3 id="faq-preview-title" className="mt-2 font-display text-2xl font-black text-[#173247]">معاينة فئة {faqPreviewLabel}</h3><p className="mt-2 text-sm leading-7 text-[#68747a]">هذه معاينة داخلية قبل الحفظ والنشر. أي مسودة ظاهرة هنا لن تظهر للزوار إلا بعد الضغط على زر الحفظ.</p></div><button type="button" onClick={() => setFaqPreviewCategory(null)} aria-label="إغلاق المعاينة" className="rounded-full p-2 text-[#68747a] transition hover:bg-[#f4eadb] hover:text-[#173247]"><X className="h-5 w-5" /></button></div><div className="mt-5 space-y-3">{faqPreviewItems.length ? faqPreviewItems.map(item => <article key={`${item.id}-${item.question}`} className={`rounded-2xl border p-4 ${item.isDraft ? "border-[#b9854a] bg-[#fffaf1]" : "border-[#e3d9ca] bg-white"}`}><div className="flex flex-wrap items-center justify-between gap-2"><h4 className="font-bold leading-7 text-[#173247]">{item.question}</h4>{item.isDraft && <span className="rounded-full bg-[#d5a15f] px-2.5 py-1 text-[10px] font-black text-[#173247]">مسودة غير منشورة</span>}</div><p className="mt-2 text-sm leading-7 text-[#68747a]">{item.answer}</p></article>) : <div className="rounded-2xl border border-dashed border-[#d7c09b] bg-[#fffaf1] p-8 text-center text-sm leading-7 text-[#68747a]">لا توجد أسئلة في هذه الفئة حالياً. يمكنك إضافة سؤال من محرر FAQ ثم معاينته هنا قبل نشره.</div>}</div><div className="mt-6 flex justify-end"><button type="button" onClick={() => setFaqPreviewCategory(null)} className="rounded-full bg-[#173247] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#24485f]">العودة إلى المحرر</button></div></div></div>}</>}
    </>}
    </div>;
}
...
function QuizManager({ chapters, selectedId, questions, onSelect, onChange, onAdd, onRemove, onSave, saving, valid }: { chapters: Array<any>; selectedId?: number; questions: QuizEditorQuestion[]; onSelect: (item: any) => void; onChange: (index: number, patch: Partial<QuizEditorQuestion>) => void; onAdd: () => void; onRemove: (index: number) => void; onSave: () => void; saving: boolean; valid: boolean }) {
  ...
  const [successMessage, setSuccessMessage] = useState("أحسنت، لقد بلغت حد النجاح. واصل القراءة والتحليل والمراجعة.");
  const [failureMessage, setFailureMessage] = useState("تحتاج إلى مراجعة إضافية. أعد قراءة المحور وحلل المفاهيم ثم أعد المحاولة.");
  ...
  const savePassingPercentage = trpc.admin.saveSetting.useMutation({
    onSuccess: () => {
      toast.success("تم حفظ إعداد الاختبار");
  ...
  useEffect(() => {
    ...
    const savedSuccess = settingsQuery.data?.find(item => item.settingKey === "quizSuccessMessage")?.settingValue;
    const savedFailure = settingsQuery.data?.find(item => item.settingKey === "quizFailureMessage")?.settingValue;
    if (savedSuccess) setSuccessMessage(savedSuccess);
    if (savedFailure) setFailureMessage(savedFailure);
  }, [settingsQuery.data]);
  ...
  const saveQuizMessage = (settingKey: "quizSuccessMessage" | "quizFailureMessage", settingValue: string) => {
    if (settingValue.trim().length < 10) {
      toast.error("اكتب رسالة لا تقل عن 10 أحرف");
      return;
    }
    savePassingPercentage.mutate({ productCode: DEFAULT_PRODUCT_CODE, settingKey, settingValue: settingValue.trim(), description: settingKey === "quizSuccessMessage" ? "رسالة النجاح في اختبار اختبر فهمك" : "رسالة الرسوب في اختبار اختبر فهمك" });
  };
            ...
            <div className="rounded-2xl border border-[#e3d9ca] bg-[#fffaf2] p-4">
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2" aria-live="polite">
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4">
                  ...
                  <p className="mt-1 text-xs text-emerald-800">{livePassedScore} من {livePreviewTotal} إجابات صحيحة، والحد المطلوب {safePassingPercentage}%.</p><p className="mt-3 text-xs leading-6 text-emerald-900">{successMessage}</p>
                </div>
                <div className="rounded-2xl border border-amber-200 bg-amber-50/70 p-4">
                  ...
                  <p className="mt-1 text-xs text-amber-900">{liveFailedScore} من {livePreviewTotal} إجابات صحيحة، والحد المطلوب {safePassingPercentage}%.</p><p className="mt-3 text-xs leading-6 text-amber-950">{failureMessage}</p>
                </div>
              </div>
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50/50 p-4">
                  <label className="block text-sm font-bold text-emerald-950">رسالة النجاح
                    <textarea value={successMessage} onChange={event => setSuccessMessage(event.target.value)} maxLength={500} className="mt-2 min-h-24 w-full rounded-xl border border-emerald-200 bg-white px-3 py-2 text-sm leading-7 text-[#173247] outline-none focus:border-emerald-500" aria-label="رسالة النجاح" />
                  </label>
                  <div className="mt-3 flex items-center justify-between gap-2"><span className="text-[11px] text-emerald-800">{successMessage.length}/500 حرفاً</span><button type="button" onClick={() => saveQuizMessage("quizSuccessMessage", successMessage)} disabled={savePassingPercentage.isPending || successMessage.trim().length < 10} className="rounded-full bg-emerald-700 px-4 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">حفظ رسالة النجاح</button></div>
                </div>
                <div className="rounded-2xl border border-amber-200 bg-amber-50/50 p-4">
                  <label className="block text-sm font-bold text-amber-950">رسالة الرسوب أو المواساة
                    <textarea value={failureMessage} onChange={event => setFailureMessage(event.target.value)} maxLength={500} className="mt-2 min-h-24 w-full rounded-xl border border-amber-200 bg-white px-3 py-2 text-sm leading-7 text-[#173247] outline-none focus:border-amber-500" aria-label="رسالة الرسوب" />
                  </label>
                  <div className="mt-3 flex items-center justify-between gap-2"><span className="text-[11px] text-amber-800">{failureMessage.length}/500 حرفاً</span><button type="button" onClick={() => saveQuizMessage("quizFailureMessage", failureMessage)} disabled={savePassingPercentage.isPending || failureMessage.trim().length < 10} className="rounded-full bg-amber-700 px-4 py-2 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">حفظ رسالة الرسوب</button></div>
                </div>
              </div>
            </div>
