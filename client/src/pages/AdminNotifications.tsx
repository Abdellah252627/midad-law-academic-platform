...
function AdminNotificationsContent() {
  ...
  const markAllRead = trpc.admin.markNotificationsRead.useMutation({
  ...
  const visibleUnreadIds = rows.filter(row => !row.isRead).map(row => row.id);
  ...
  return <section dir="rtl" className="mx-auto max-w-7xl space-y-6">
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-[#e3d9ca] bg-white px-4 py-3 shadow-sm"><p className="inline-flex items-center gap-2 text-sm text-[#68747a]"><Filter className="h-4 w-4 text-[#b9854a]" aria-hidden="true" /> يطبق الإجراء على النتائج الظاهرة في الصفحة الحالية فقط.</p><button type="button" disabled={markAllRead.isPending || visibleUnreadIds.length === 0} onClick={() => markAllRead.mutate({ ids: visibleUnreadIds })} className="rounded-xl bg-[#173247] px-4 py-3 text-sm font-bold text-white transition hover:bg-[#24465e] disabled:cursor-not-allowed disabled:opacity-50">{markAllRead.isPending ? "جارٍ التحديث…" : "تحديد الظاهر كمقروء"}</button></div>
